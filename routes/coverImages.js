...
const Cover = require('../models/coverImages')
// const coverImages = require('../models/coverImages')
...
router.get('/getImages', async (req, res) => {
  try {
    ...
    const allProducts = await Cover.findById('67d836ecd3754c027ae211e4')
...
router.post('/initializeImages', async (req, res) => {
  try {

    const newProduct = await Cover.create(req.body)
    res.send(newProduct)
...
router.post('/editImages/:id', async (req, res) => {
  try {

    const newProduct = await Cover.findByIdAndUpdate(req.params.id,{$set:req.body},{new:true})
    res.send(newProduct)
...
//     const newProduct = await Rooms.findByIdAndDelete(req.params.id)
//     res.send(newProduct)
