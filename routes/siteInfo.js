...
//     const coverImages = await SiteInfo.find()
...
//   }
// })
// 6796b5cba8a69e5930a18494
router.get('/getSite', async (req, res) => {
  try {

    const allProducts = await SiteInfo.findById('67d835eb76642693ef68a45f')
...
router.post('/initializeSite', async (req, res) => {
  try {
    ...
    const newProduct = await SiteInfo.create(req.body)
    res.send(newProduct)
...
router.post('/editSite/:id', async (req, res) => {
  try {

    const newProduct = await SiteInfo.findByIdAndUpdate(req.params.id,{$set:req.body},{new:true})
    res.send(newProduct)
...
router.delete('/deleteSite/:id', async (req, res) => {
  try {
    ...
    const newProduct = await SiteInfo.findByIdAndDelete(req.params.id)
    res.send(newProduct)
